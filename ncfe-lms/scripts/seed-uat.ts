...
async function seed() {
  console.log('╔══════════════════════════════════════════════════╗');
  console.log('║         NCFE LMS — UAT Seed Script              ║');
  console.log('╚══════════════════════════════════════════════════╝');
  console.log('');
  ...
  let centre = await Centre.findOne({ code: 'LE001' });
  if (!centre) {
    centre = await Centre.create({
      name: 'Learners Education',
      code: 'LE001',
      contactEmail: 'admin@learnerseducation.com',
      address: 'London, UK',
      ncfeCentreNumber: '67890',
      status: 'active',
    });
  ...
  if (!qualification) {
    ...
    console.log(`  Created qualification: ${qualification.title}`);
  } else {
    ...
    console.log(`  Qualification already exists: ${qualification.title}`);
  }
  ...
  const usersData = [
    { name: 'Jyothi', email: 'jyothi@learnerseducation.com', role: 'assessor', password: 'password123' },
    { name: 'Vidit', email: 'bhatnagar007vidit@gmail.com', role: 'student', password: 'password' },
    { name: 'Peter', email: 'intern@learnerseducation.com', role: 'student', password: 'password' },
    { name: 'Bruce', email: 'iqa@test.com', role: 'iqa', password: 'iqapassword' },
  ];
  ...
  for (const userData of usersData) {
    ...
    if (!user) {
      // Hash each user's password individually
      const hashedPassword = await bcrypt.hash(userData.password, 12);

      // Insert directly to bypass the pre-save hook (which would double-hash)
  ...
  const studentUsers = [
    { user: createdUsers['bhatnagar007vidit@gmail.com'], name: 'Vidit' },
    { user: createdUsers['intern@learnerseducation.com'], name: 'Peter' },
  ];

  for (const { user: studentUser, name } of studentUsers) {
    ...
    if (!enrolment) {
      ...
      console.log(`  Created enrolment: ${name} -> ${qualification.title}`);
      console.log(`    Assessor: Jyothi, Cohort: 2026-Q1, Status: in_progress`);
    } else {
      console.log(`  Enrolment already exists for ${name}`);
    }
  }
  ...
  console.log(`  Slug: ${qualification.slug}`);
  console.log(`  Units: ${unitsData.length}`);
  console.log(`  Users: ${usersData.length}`);
  console.log(`  Enrolments: 2 (Vidit + Peter under Jyothi)`);
  console.log('');
  console.log('Login credentials:');
  console.log('  Assessor:  jyothi@learnerseducation.com / password123');
  console.log('  Student 1: bhatnagar007vidit@gmail.com  / password');
  console.log('  Student 2: intern@learnerseducation.com / password');
  console.log('  IQA:       iqa@test.com                 / iqapassword');
  console.log('');
  console.log(`Assessor dashboard: /c/${qualification.slug}`);
